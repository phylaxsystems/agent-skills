...
- Deploying or activating an assertion release on-chain with `pcl`, fully
  agentically (no browser wallet)
- Setting up pcl for deployment: auth, RPC endpoints, wallet/keystore
- Setting or transferring a project's protocol manager
- Deactivating (removing) a deployed release

Requires a `pcl` with `pcl deploy` and `--broadcast` support — verify with
`pcl deploy --help`; older versions only print calldata for external signing.

## When NOT to Use

- You are designing or writing the assertions themselves. Use
  `write-protection-assertions` or `designing-assertions`.
- You only need local testing or backtesting. Use `testing-assertions` or
  `backtesting-assertions`.
- You want the full authoring lifecycle walkthrough (project bootstrap, test,
  store, submit). Use `pcl-assertion-workflow`; come back here for the
  on-chain deployment leg.
