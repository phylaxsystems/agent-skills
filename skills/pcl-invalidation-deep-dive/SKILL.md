...
4. **Assemble the local triage context**
   ...
   - When `contract_context_manifest.json` has bytecode-backed `decompiler_targets`, run `scripts/run_decompiler.py contract_context/contract_context_manifest.json --out-dir decompiled --require-success` after configuring a backend. Prefer `heimdall`/Heimdall-rs, a licensed JEB headless/API wrapper, or an internal JSON decompiler API via `HEIMDALL_DECOMPILE_CMD`, `JEB_DECOMPILE_CMD`, `EVM_DECOMPILER_CMD`, `DECOMPILER_CMD`, `EVM_DECOMPILER_API_URL`, or `DECOMPILER_API_URL`. Use `scripts/run_dedaub_decompiler.py` only when Dedaub programmatic API entitlement is confirmed.
...
7. **Replay and verify on chain**
   ...
   - For code-bearing addresses without verified source, use a configured EVM decompiler backend. Prefer a controllable backend: local Heimdall-rs, an internal REST wrapper around Heimdall, or licensed JEB headless/API automation. Treat browser-only decompiler sites as manual references, not production dependencies. If using native Dedaub, `DEDAUB_API_KEY` is required and must have programmatic API entitlement; `DEDAUB_API_URL` is only an optional base-url override. If no decompiler backend is configured, store runtime bytecode and list the address as a decompiler target.
...
- Optional EVM decompiler backend for unverified contracts. Preferred options are `heimdall`/Heimdall-rs on PATH, `HEIMDALL_DECOMPILE_CMD`, a licensed JEB wrapper via `JEB_DECOMPILE_CMD`, a custom command wrapper via `EVM_DECOMPILER_CMD` or `DECOMPILER_CMD`, or a JSON API via `EVM_DECOMPILER_API_URL` or `DECOMPILER_API_URL`. Native Dedaub remains optional and requires `DEDAUB_API_KEY` with programmatic API entitlement; `DEDAUB_API_URL` alone is only a base-url override.
